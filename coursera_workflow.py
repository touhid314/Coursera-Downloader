#!/usr/bin/env python3
"""
Coursera Course Download Workflow Engine

This module contains the core workflow logic for downloading Coursera courses.
Originally named 'workflow.py', it was renamed to 'coursera_workflow.py' to
avoid naming conflicts with PyInstaller's internal workflow hooks.

Key Components:
- Module/Section/Lecture hierarchy iteration
- Course download orchestration
- File organization and naming
- Progress tracking and error handling
- Integration with authentication, filtering, and formatting modules

Classes:
- CourseraDownloader: Main class that orchestrates course downloads
- IterModule: Iterator for course modules with sections
- IterSection: Iterator for sections within modules containing lectures
- IterLecture: Iterator for individual lectures with resources

Functions:
- _iter_modules: Creates hierarchical structure of course content
- _walk_modules: Walks through modules and downloads content
- Various utility functions for file handling and progress tracking

This module is the heart of the download process, coordinating between:
- Authentication (cookies, login credentials)
- Content discovery (syllabus parsing, resource enumeration)
- Filtering (section/lecture/resource filters)
- Download management (parallel jobs, retry logic, progress tracking)
- File organization (directory structure, naming conventions)

Note: This file was renamed from 'workflow.py' to resolve PyInstaller build
conflicts. All imports in other modules have been updated accordingly.

Author: Original Coursera-Downloader project contributors
License: Same as parent project
Renamed for: Native Windows CLI compatibility
"""
...
def _iter_modules(modules, class_name, path, ignored_formats, args):
    """
    Generate hierarchical structure of course modules, sections, and lectures.
    
    This function creates a structured representation of a Coursera course
    by processing the raw module data and applying user-specified filters.
    It builds an iterator hierarchy that allows for organized traversal
    of the course content during download.
    
    The hierarchy structure is:
    Course -> Modules -> Sections -> Lectures -> Resources
    
    Args:
        modules: Raw module data from course syllabus
        class_name (str): Course class name for directory organization
        path (str): Base download path
        ignored_formats (list): File formats to skip during download
        args: Command-line arguments object containing:
            - file_formats: Specific formats to download
            - lecture_filter: Regex filter for lecture names
            - resource_filter: Regex filter for resource names  
            - section_filter: Regex filter for section names
            - verbose_dirs: Whether to use verbose directory names
            - combined_section_lectures_nums: Number formatting options
    
    Returns:
        Generator yielding IterModule objects that contain:
        - Filtered and organized sections
        - Properly named directories following naming conventions
        - Lecture content filtered by user preferences
    
    Features:
        - Applies regex filters for sections, lectures, and resources
        - Creates standardized directory naming (01_section-name format)
        - Handles verbose vs. compact directory structures
        - Integrates with resource filtering and format selection
        - Supports combined section/lecture numbering schemes
    
    The function creates nested iterator classes that maintain the course
    structure while allowing efficient traversal during download:
    - IterModule: Represents a course module (week/chapter)
    - IterSection: Represents a section within a module
    - IterLecture: Represents individual lectures with downloadable content
    """
